...
// UGLY CODE I KNOW BUT I DONT CARE


	$titleImportance = is_front_page() ? 1 : 2;

	/* Get the site description.  If it's not empty, wrap it with the appropriate HTML. */
	if ( $desc = get_bloginfo( 'description' ) ) {
		$descEl = 'h'.($titleImportance + 1);		
		$desc = sprintf( '<' . $descEl . ' class="site-description"><span>%1$s</span></'.$descEl.'>', $desc );
	}

	$title = get_bloginfo('name');
	$titleEl = 'h'.$titleImportance;
		
		if ( $logo = get_theme_mod( 'custom_logo' ) ) {
	?>

<div class="<?= omega_apply_atomic( 'title_area_class', 'title-area') ?>">

<div itemscope itemtype="http://schema.org/Organization" class="site-header">
				
				<img itemprop="logo" alt="<?= esc_attr( $title ) ?>" src="<?= $logo ?>"/>

				<a itemprop="url" href="<?= home_url() ?>" title="<?= esc_attr( $title ) ?>" rel="home">
					<<?= $titleEl ?> class="site-title" itemprop="headline"><?= $title ?></<?= $titleEl ?>>
					<?= $desc ?>
				</a>
			</div>
